...
public class ProdutoJDBCDAO implements ProdutoDAO {
	...
	public void save(Produto entity) {
		...
		try {
			...
			String insert_sql = "insert into produto (codigo, descricao, preco, qtdEstoque) values (?, ?, ?, ?)";
			String update_sql = "update produto set codigo = ?, descricao = ?, preco = ?, qtdEstoque = ? where id = ?";
	...
	public List<Produto> findByDataUltimaEntrada(Date dataInicial, Date dataFinal) {
		...
		try {
			con = ds.getConnection();
			PreparedStatement pst;
			String sql = "select * from produto where dataUltimaEntrada between cast(? as timestamp) and cast(? as timestamp)";
			pst = con.prepareStatement(sql);
